...
7. **Implement**: `dk-implementer` in the locked worktree, or `agent-run run --role implementer --cwd <wt> --brief brief.md`. Two in parallel: `--detach` + `agent-run wait <id>` externally, background dispatch natively.
   ...
   **Polling a detached run: prefer `wait`, or `list` when you are watching several.** All three commands reconcile a supervisor that died without recording its result, so none of them can report a corpse as still running. A run whose supervisor died comes back as `orphaned`, or `timeout` when the clock ran out, and its `result.json` says which. Both are terminal — but neither means the work is lost: a killed worker has usually committed before dying, so read the worktree before you rerun anything.
