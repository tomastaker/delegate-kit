...
Materialize these profiles only after the user selects exact supported executor settings. Preserve the bundled compatible default and existing saved `main.json`; no economy model, account, reasoning level, alternative provider or subscription is selected automatically. One model may serve multiple tiers. Teams without tiers, extra researchers, hard profiles or diverse model families remain valid. Measure cost per accepted task with verification and rework before proposing tuning; incomplete usage remains unknown and small samples never rewrite presets automatically.
