...
The existing `resources` claims coordinate active workers; they do not discover outside processes or own services left running after a worker stops. Project launch/check scripts must manage their own cleanup, including failure handling. Delegate Kit is not a general service supervisor.
