...
ok "reviewer после codex-автора → claude, независим, нативно" "$(route both --role reviewer --parent claude --author-backend codex '.reviewers[0]|[.backend,(.independent|tostring),.dispatch]|join(" ")')" "claude true native"
ok "verifier — третья сторона к ревьюеру: обратно на codex" "$(route both --role verifier --parent claude --author-backend codex '.backend + " " + (.why[0]|contains("third party")|tostring)')" "codex true"
...
ok "main-claude: ревьюер следует за автором → codex" "$(route both --role reviewer --parent claude --preset main-claude --author-backend claude '.author + " " + .reviewers[0].backend')" "claude codex"
...
ok "--kind ui: ревьюер следует за UI-автором → codex" "$(route both --role reviewer --parent claude --kind ui --author-backend claude '.author + " " + .reviewers[0].backend')" "claude codex"
...
echo "── автор обязателен для reviewer и verifier"
ok "route reviewer без автора — отказ" "$(PATH="$BASE/both" "$NODE" "$AR" route --role reviewer --parent claude 2>&1 | grep -c 'author-backend claude|codex|self is required')" "1"
ok "route verifier без автора — отказ" "$(PATH="$BASE/both" "$NODE" "$AR" route --role verifier --parent claude 2>&1 | grep -c 'is required for --role verifier')" "1"
