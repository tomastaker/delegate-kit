...
ok "Coordinator can launch a worker" "$(gate claude "" 'dk.mjs run --role reviewer --backend codex --brief b.md')" "allow"
ok "Worker cannot launch another worker" "$(gate claude "dk-implementer" 'dk.mjs run --role reviewer --backend codex --brief b.md')" "deny"
ok "Depth reason names the worker" "$(reason | grep -c 'depth is 1.*dk-implementer')" "1"
ok "Worker cannot resume another worker" "$(gate claude "dk-planner" 'dk.mjs resume 2026-x --brief n.md')" "deny"
ok "Worker cannot lock a worktree" "$(gate claude "dk-implementer" 'dk.mjs prepare --task slice-a')" "deny"
# The command is input to the gate parser, so preserve its literal tilde.
# shellcheck disable=SC2088
ok "Tilde helper path is covered" "$(gate claude "dk-implementer" '~/.claude/skills/delegate-kit/scripts/dk.mjs run --role planner --brief b.md')" "deny"
ok "Quoted executable is covered" "$(gate claude "dk-implementer" '"dk.mjs" run --role planner --brief b.md')" "deny"
ok "Quoted helper path is covered" "$(gate claude "dk-implementer" "'/home/me/bin/dk.mjs' prepare --task task")" "deny"
ok "env and node wrappers are covered" "$(gate claude "dk-implementer" 'env DELEGATE_KIT_PRESET=auto node /x/dk.mjs run --role planner --brief b.md')" "deny"
ok "Command chains are covered" "$(gate claude "dk-implementer" 'cd /repo && dk.mjs run --role planner --brief b.md')" "deny"
ok "Confirmation cannot bypass delegation depth" "$(gate claude "dk-implementer" 'DELEGATE_KIT_CONFIRMED=1 dk.mjs run --role planner --brief b.md')" "deny"
ok "Codex worker is also denied" "$(gate codex "dk-reviewer" 'dk.mjs run --role verifier --brief b.md')" "deny"
ok "Worker can inspect status and wait" "$(gate claude "dk-reviewer" 'dk.mjs status 2026-x; dk.mjs list; agent-wt status s')" "allow"
...
for action in open check submit escalate disposition accept; do
  ok "Worker cannot mutate task $action" "$(gate claude "dk-implementer" "node /installed/delegate-kit/scripts/dk.mjs task $action --session x --task t")" "deny"
done
ok "Worker cannot certify a checkpoint" "$(gate codex "dk-implementer" 'node /installed/delegate-kit/scripts/dk.mjs verify checkpoint --check tests')" "deny"
ok "Worker cannot create a checkpoint" "$(gate claude "dk-implementer" 'node /installed/delegate-kit/scripts/dk.mjs checkpoint create --session x --task t')" "deny"
ok "Worker can inspect task state" "$(gate claude "dk-reviewer" 'node /installed/delegate-kit/scripts/dk.mjs task show --session x --task t')" "allow"
ok "Coordinator can accept a task" "$(gate claude "" 'node /installed/delegate-kit/scripts/dk.mjs task accept --session x --task t')" "allow"
